...
形成或改写面向用户的分析报告前，完整读取[专业投研正文与必要归因](../domi-router/references/professional-investment-writing.md)；本任务已读过同一版本时复用。仅取数、计算或维护控制件时不额外加载。该规范适用于以下全部研究模式，只改变表达，不减少模型、财务明细、证据与质量门，也不把材料整理自动升级为深度研究。
...
控制件完整保留，正文按投资问题与有依据的结论组织，连接证据、经营机制、财务/估值影响和反证；不按受访者、研报或新闻来源逐篇摘要。核心指标、假设与模型表继续展示数值、单位、期间、实体口径及来源，不把内部 claim_id、审计状态和置信度列机械搬成正文。公司自报、管理层指引、市场预期、我们的估算与未解决冲突在相关句子或表列就近区分，不因改为业务主语而升级成已实现或独立核实的事实。
...
摘要只提炼决定判断的结论与关键数字；明细表承载比较，正文解释机制与差异，不重复全文数据。各小节先回答对应投资问题，关键直接引语仅在原话措辞对判断有意义时保留，真实来源引用和日期仍随事实呈现。交付前语义审核须确认改写未丢重要机制、细节、责任主体、限定与反证，不能用流畅度或关键词替代证据审核。
