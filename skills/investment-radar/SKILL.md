...
- 最后报告扫描窗口、后端、覆盖缺口，以及 `created / updated / unchanged / skipped / failed` 数量和资料库回执。
...
一次扫描只有在范围明确、重点项目快照与重点人脉已加载或记录明确覆盖缺口、分类已复用或获得明确 taxonomy-sync 状态、原文已打开、事件完成重点对象匹配、去重与评分、每条事件获得明确写入或拒绝状态、成功写入项已回读验证、用户先收到重点对象动态再收到一般值得关注项与归档回执后才算完成。无有效新闻也可以完成，但必须保留候选与拒绝原因统计；Base、重点对象读取或 taxonomy-sync 失败时只能报告“搜索完成、归档未完成”或具体部分完成状态。
