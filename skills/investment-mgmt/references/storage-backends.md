...
行业主导的播客纪要不放在按月行业事件目录，而作为可长期维护的行业研究主文档保存：

```text
<localRepositoryDir>/1.行业研究/<领域>/<主子领域>/播客/<节目名称>/
├── <YYYYMMDD>-<单集主题>.md          # 唯一可编辑主纪要
└── <YYYYMMDD>-<单集主题>-PLAUD文字稿.md  # 需要保留完整稿时创建一次
```

公司主导的播客主纪要进入对应既有项目的 `纪要/`，PLAUD 文字稿可在同一项目的 `原始材料/` 保存一次。一个单集只能有一个 `canonicalDocumentId` 和一个可编辑主纪要；行业动态事件、项目主页、人物主页和另一侧行业／项目索引只保存 URI、摘要和关系，不复制正文。归档判定、公开下载和 PLAUD 门禁遵循 `../../investment-radar/references/podcast-ingestion.md`。

如果当前后端没有可写的唯一行业研究根目录，保留播客产物在 Application Support 私有处理队列并提示配置；不得从飞书项目材料目录的父级猜测工作区，也不得静默切换后端。
