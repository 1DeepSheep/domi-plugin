...
- **只读连接与写入授权是两层概念**：连接完成后，项目、人物、PLAUD 录音和研究工作流可以围绕当前已识别实体调用 `lark-wiki`、`lark-doc`、`lark-base` 做窄范围只读参考；创建、编辑、更新、覆盖、移动、发布、上传、发送及收件人解析仍须本轮用户明确指令。只读能力绝不自动扩张成写入授权。
...
| 飞书能力 | Skill | 连接后允许的动作 | 与本地主库的关系 |
|---|---|---|---|
| Base | `lark-base` | 按当前实体窄范围搜索／读取既有 Base、表、视图、字段和记录；本轮明确指定目标与变更时才可创建或编辑外部 Base 内容 | 仅作外部参考或用户指定的协作产物，不成为项目／人脉／行业／待办管理账本 |
| Wiki | `lark-wiki` | 按当前实体浏览／搜索空间与节点；本轮明确要求时才创建、移动或整理节点 | 是外部知识空间或发布位置，不替代本地目录 |
| Docs | `lark-doc` | 读取相关飞书文档；本轮明确要求时才创建或编辑 | 是参考文档或发布副本，不替代本地 Markdown |
| Drive | `lark-drive` | 按当前实体搜索／读取文件；本轮明确要求时才上传、移动或整理文件 | 是外部文件来源或交付位置，不替代本地附件目录 |
...
- 或飞书已连接，当前属于项目、人物、PLAUD 录音或研究工作流，且已经得到可用于消歧的公司名、主体名、产品名、人名或必要别名。此时可以主动做窄范围只读检索，把命中内容作为补充参考。

执行规则：

1. 完整采用 `lark-shared`；Base 使用 `lark-base`，知识空间使用 `lark-wiki`，文档正文使用 `lark-doc`，云盘文件使用 `lark-drive`，统一使用当前用户身份。
2. 搜索词只使用当前实体的规范名、主体名、产品名及必要别名；先搜标题／节点／表记录，再读取唯一或高置信度相关内容。不要做无实体、跨主题或全空间漫游，不要求预先绑定某个 Base 或 Wiki Space。多个同名候选无法消歧时跳过自动读取；用户明确要求查找时再展示资源类型、标题／表名、所属空间和更新时间供其选择。
...
5. 自动只读参考是非必需增强：未连接、无命中、歧义、权限或网络失败时直接继续本地主流程，不得切换、重建或清空本地资料库，也不得把飞书检索列为未完成项。用户明确要求读取的目标失败时，才报告对应读取失败。

## 3. 明确命令创建或编辑飞书外部内容

只有本轮用户原始消息明确要求对应写动作时，才设置 `feishu_knowledge_action=create|edit`，并兼容已有的 `delivery_only=feishu_doc`。既往消息、旧队列字段、已有 Wiki／Doc 链接、飞书连接、只读搜索命中、本地未命中或“继续处理”都不是写入授权：
...
- 没有本轮明确飞书写指令时，省略整个外部写入阶段：不运行 Markdown 导出预检／handoff，不创建交付任务，不返回 `FEISHU_EXPORT_HANDOFF_REQUIRED`，也不把飞书副本写成未完成项。

## 4. Markdown → 飞书的保真搬运

本轮用户说“把本地 Markdown 搬到／复制到飞书文档”时，必须把源 Markdown 视为权威源，并完整采用 `lark-doc` 的 Markdown 创建／更新规则。禁止先把文档渲染成纯文本再写入。没有这类本轮写指令时不得进入本节。
