...
- `项目名称`／`name` 使用已确认的品牌／项目简称，工商全称进入 `legalName`，旧名和中英文别名进入 `aliases`；创建、改名、归档与查重前读取[项目命名规则](references/project-names.md)。会议日期、产品／技术主题、评级和进展属于文档标题或结构化字段，禁止把 `YYYYMMDD-主体-主题-A` 一类归档标题写成公司名称。旧目录只可提供候选名；没有主页、Base 记录、独立材料或用户确认时必须进入名称审核，不得从目录名自动盖章。
...
`项目主页.md` 是项目的快速决策入口，不是数据库字段转储。首屏必须按“项目名称 → 领域／进展／评级 → 打开项目目录 → 投资摘要 → 项目概览”排列；系统 schema、实体 ID 和同步字段只保留在 frontmatter，由客户端默认隐藏。项目概览使用人类可读日期和空值，不直接展示 ISO 时间、`null` 或内部 ID。

主页默认关联材料只突出会议纪要、投资快评、深度研究、BP／Datapack 和 IC 材料，并提供“打开项目目录”入口。PLAUD 文字稿、录音及其他底层证据继续保留在 `原始材料/`，但不在项目主页常用材料中展示；需要事实核验时再进入目录查看。
...
项目搜索匹配名称、工商名和别名，人物搜索只匹配姓名和组织；`searchCoverage` 标明实际匹配范围，`completeScope=pagination` 表示 `complete` 只管分页。空结果不能推出文档正文没有提及；当前网关不提供全文检索，正文未查时须保留这一范围限制。
