...
| 用户目标 | 模式与主链路 | 按需读取 |
|---|---|---|
| 产品叫什么／属于谁／还有哪些产品等单点事实 | 直接执行下方「简短事实查证」 | 不加载完整项目／人物研究链路 |
...
单点问题直接查证并回答，不因“查一下”启动完整研究、评级、入库或多阶段 manifest。

- 用户给出链接时先读正文；未知答案从姓名、公司或已有履历线索检索，转写疑名只作候选。
- 首轮没有可靠答案时，沿已知任职／曾负责产品发现花名或别名，或换一种可用来源补查。须实际尝试有依据的新路径，不能把反复改写同组关键词当成补漏；新线索继续核验，已足够回答就停止扩展。
- 打开相关正文核验所问关系并附来源；曾负责、团队推出、公司运营和商店发行分别归属。仍无有效线索时只报告已查范围内未找到；实体字段查询不代表正文查询，读取失败不代表无记录，不把未查范围写成否定结论。
