...
确认升级后复用已完成的实体核验、研究底稿、信源和事实状态，直接从 `investment-review` 开始，随后按 `storage-backends.md` 写入当前权威资料库；不得重新做一遍桌面研究。用户若只要求评级、不要求入库，则只执行 `investment-review`，继续禁止资料库写入。
...
| 内部查重（仅 `intake/update`） | `domi:investment-mgmt` | 指纹与别名 | `project_id?`、`document_uri?` | 本地主库查 SQLite／项目目录；旧飞书主库查 Watching List／Wiki |
...
| 文档与项目库归档 | `domi:investment-mgmt` | 底稿、快评、分类、用户材料 | 当前后端 `document_uri`、`library_path`、归档清单 | 按后端回读 Wiki／Markdown 与材料目录 |
...
- 本地主库用 `project upsert` 创建稳定项目目录和 `项目主页.md`，用 `document create` 保存研究、纪要和快评，原始材料进入 `原始材料/`。只有用户明确要求创建／编辑飞书文档交付副本时，才另行采用 `feishu-knowledge-extension.md`，且本地归档必须先成功。
...
1. 以核心产品／主要收入来源确定一个主子领域目录；其他子领域保留在当前项目记录的多值分类字段中。
...
9. 列出材料目录并核验关键二进制文件大小或校验和。当前后端文档 URI、结构化记录与项目路径均取得并验证后，文档归档阶段才算完成。
...
仅在 `intake` 或 `update` mode 执行。采用并完整遵循 `domi:investment-mgmt` 的最新字段 schema、taxonomy 与状态规则。本地主库写入 SQLite `projects`；旧飞书主库按 legacy reference 写 Watching List。两者都只在文档与材料成功归档后一次性写入：
...
- [ ] 项目文档和材料目录位于当前后端的正确领域／主子领域，且源材料和产物已写后验证；
...
每个阶段保留 `entity_fingerprint`、任务开始时锁定的 `repositoryBackend`、内部 `project_id/record_id`、`document_uri`、`library_path` 和研究结论，失败后从当前阶段恢复。恢复状态只用于内部续跑，不进入默认用户报告：
...
| 文档与目录成功、结构化写入失败 | 复用当前 URI、项目路径和研究结果，按后端重试 SQLite／Base upsert |
