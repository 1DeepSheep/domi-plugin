...
4. 把项目简称、工商全称与材料／纪要标题分开：完整读取[项目命名规则](../../investment-mgmt/references/project-names.md)，结构化 `name` 使用已确认的品牌／项目简称，工商全称写 `legalName`，旧名及中英文别名写 `aliases`，用这些字段查重并复用稳定 ID；日期、产品／技术主题、评级只进入文档标题和对应字段。遇到日期型旧目录时必须用用户确认、已有主页／Base 记录或独立材料核验，不能直接把目录名当公司名。
