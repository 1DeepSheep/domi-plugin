...
- 用户明确要求把某个项目加入 Watching List：走项目 intake。
...
Router 不自行判断或直接修改 Base schema，只负责交接和恢复。

### 3. 归档

Radar 把全部通过采纳门槛的事件写入《1.2 行业信息追踪》，而不是只写最终推荐项。Router 不直接操作 Base，也不在 Radar 之外再次创建记录。

如果检索完成但 Base 不可访问、字段不一致或写后无法验证，保留研究结果并标为“搜索完成、归档未完成”；不得声称已经写入，也不得转写到 `1.0 项目Watching List`。
...
- “研究并入库／加入 Watching List”：在研究完成后进入项目 intake，并按其去重、文档归档和明确写入授权执行。
- “继续关注”：保留在 Radar 事件库，不自动创建项目或修改 Watching List。
