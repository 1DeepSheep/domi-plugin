...
1. 从 `$DOMI_CONFIG_PATH` 读取 `wikiSpaceId`，在该 Wiki 空间中按公司中文名、英文名、产品名和主体名查重；同时排除聚合文档和同名无关项目。配置缺失时停止并提示用户到豆米“设置 → 资料连接”补充。
2. 唯一明确匹配时复用既有 `wiki_node_token` 和 `doc_token` 更新正文，保留用户已有的独立章节、图片、附件和产品更新；不得再建同名节点。
...
6. 写入后执行 `docs +fetch` 核验标题、参会人、核心章节、评分／评级和关键数字；再用 Wiki `get_node` 核验父节点链确实位于目标行业目录。
