...
- 用户明确回复`管理层访谈`、`创始人访谈`或创业公司标准访谈时，把对话类型、项目名、已知赛道和项目方参会人的姓名／职位传给`domi:asr-notes`。所有项目交流的纪要文件名、文档标题（`####`）和归档显示标题统一按[文件命名与交付规则](../../asr-notes/references/delivery-rules.md)使用`YYYYMMDD-项目规范名-所属赛道`，例如`20260115-示例科技-具身智能`；姓名、职位写入参会人行，不替代赛道。录音自动标题仅作来源线索。
...
6. 按[全源覆盖检查](../../asr-notes/references/source-coverage.md)从完整原文建立信息清单，保留每个源区间的真实正文落点／合理排除；完成实体、数字、学历分层、履历、归因、句内冲突、完整性和编辑审查，再绑定格式化后最终文件的哈希。审计表、冲突清单和定位只进入内部证据工件；纪要按asr-notes直接呈现事实，必要分歧在对应事实旁表达一次，不追加口径说明、审计附录或下游快评／尽调建议。不得用空话或“未独立核验”备注替代会中明确事实。
7. `project` 项运行：
