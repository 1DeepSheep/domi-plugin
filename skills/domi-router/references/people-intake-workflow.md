...
若用户已经给出一个明确人物，跳过广泛 `discover`，直接执行身份核验与 `profile`；这不省略未命中时的别名／履历补查。单点事实问题按 [Router 的简短事实查证](../SKILL.md#简短事实查证) 回答。若用户要求人才池、若干候选或某赛道潜在创始人，必须先 `discover` 再为 Included 候选补齐最小可入库画像。
...
每次写入前展示或记录完整字段 diff；系统型 `入库时间` 不计入可写 diff。每条写后按 `person_id` 回读，验证姓名、主类型、组织身份、来源、关系状态、下一步以及系统生成的入库时间。涉及关联项目时还要验证链接对象唯一正确。

## 七、批量确认、幂等与恢复

批量／开放式候选确认表至少包含：人物、指纹摘要、Included 理由、计划动作、目标 `person_id?`、关键字段 diff、跳过或歧义原因。只有用户确认的 create／update 项可以写；Watchlist、Excluded、ambiguous 和未确认项保持零写入。

恢复时保留：`workflow_id`、`mode`、`brief_hash`、`candidate_id`、`person_fingerprint`、`source_ledger`、本地 schema 版本、`dedupe_result`、`planned_action`、`confirmation_status`、`person_id`、逐记录 `write_status` 与 `verified_fields`。
...
| 批量部分失败 | 保留成功项 `person_id`，只重试 pending／failed 项，禁止整批重发 |
| 写后字段不一致 | 按 `person_id` 定向 patch 差异，不整表覆盖 |
