...
- user-managed news/RSS/official-account/podcast source lists, watch keywords, podcast download queues, temporary audio, or source watermarks;
...
Podcast ingestion only reads public, free episode pages or RSS enclosures. The plugin does not ship or collect private platform API endpoints, browser cookies, paid/private audio, DRM bypasses, downloaded audio, or PLAUD transcripts. Authorized podcast audio is uploaded only to the user’s own PLAUD account; complete audio and transcripts remain in the user’s private runtime data and library.
